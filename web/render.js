...
export function renderMain() {
  ...
  if (state.loading) html = '<p class="empty" role="status">Loading your collection…</p>';
    ...
    case 'album': html = head('THE COLLECTION / ALBUM',state.album?.name || '',state.album?.artist || '') + '<div class="hero-actions"><button class="primary" data-action="play-list">▶ Play album</button><button class="secondary" data-action="save-tracks">Save album</button></div>' + trackList(state.tracks); break;
    ...
    case 'playlist': {
      ...
      html = head('NAVIDROME PLAYLIST',p?.name || '',`${state.tracks.length} tracks · ${p?.owner || ''}`) + `<div class="hero-actions"><button class="primary" data-action="play-list">▶ Play playlist</button>${own ? '<button class="secondary" data-action="save-tracks">Copy tracks</button><button class="secondary" data-action="rename-playlist">Rename</button><button class="secondary" data-action="append-queue">Add queue</button><button class="secondary" data-action="delete-playlist">Delete</button>' : ''}</div>` + trackList(state.tracks,own); break;
    }
