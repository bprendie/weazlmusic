...
export function account() {
  modal(`<img class="auth-brand" src="weazlhead.png" alt="Weazl"><span class="eyebrow purple">YOUR LOCAL NODE</span><h2>${esc(state.user)}</h2><p>${state.admin ? 'Administrator' : 'Navidrome user'}</p><p>Navidrome: ${esc(state.connection?.url || "Configured by administrator")}<br>User: ${esc(state.connection?.username || state.user)}</p><div class="dialog-actions">${state.admin ? '<button class="secondary" id="configure-admin">Installation settings</button><button class="secondary" id="configure-connection">Configure Navidrome</button>' : ''}<button class="secondary" id="change-password">Change local password</button><button class="secondary" id="configure-llm">LLM curator</button></div><button class="primary" id="logout">Sign out</button>`);
}
